...
#'
#' @details
#' \code{ak_db_connect} takes no arguements.  It will ask for your AFSC username and password and then create and test a connection to the database.
#'
#'
#' @note The function uses \code{odbc} to connect to the database. Currently only connects to AFSC Oracle database. Might be modified in the future to include other databases, e.g., AKRO.
#' Note also that this will *not* create a connection in Rstudio.
#'
#' @seealso \code{odbc::dbConnect, odbc::dbGetQuery()}, \code{rstudioapi::askForPassword()}
#'
